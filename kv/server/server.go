...
func (server *Server) RawGet(_ context.Context, req *kvrpcpb.RawGetRequest) (*kvrpcpb.RawGetResponse, error) {
	...
	// 此处注意出错的时候不应该直接返回nil，而是返回一个标示为notfound的Response，并且err似乎始终nil?
...
func (server *Server) RawPut(_ context.Context, req *kvrpcpb.RawPutRequest) (*kvrpcpb.RawPutResponse, error) {
	...
	if err := server.storage.Write(req.Context, []storage.Modify{
		...
		// FIXME: 测试中似乎不怎么关心第一个返回值，暂时先设置为nil
		return nil, nil
	}
...
func (server *Server) RawScan(_ context.Context, req *kvrpcpb.RawScanRequest) (*kvrpcpb.RawScanResponse, error) {
	...
	for i := uint32(0); i < req.GetLimit(); i++ {
		...
		val, _ := iterator.Item().Value()
		respKvPairs = append(respKvPairs, &kvrpcpb.KvPair{
			Key:                  iterator.Item().Key(),
			Value:                val,
		})
		iterator.Next()
	}
	return &kvrpcpb.RawScanResponse{Kvs: respKvPairs}, nil
}
